...
func _shell_command_with_chdir() -> Dictionary:
	...
	match os:
		...
		"Windows":
			# In Powershell, we need to enclose the _working_dir in single
			# quotes to make it a verbatim string, and thus, need to replace
			# every single quote by adding ` upfront.
			# This also needs to be done with ‘ and ’, because Powershell
			# interprets them the same as normal quotation marks.
